...
float noise( in vec2 p )
{
    ...
    //return mix(sin(p.x)*sin(p.y),cos(p.x)*sin(p.y),sin(p.x)*cos(p.y));
}


//const mat2 mtx = mat2( 0.4,  0.4, -0.2,  0.2 );
//const mat2 mtx = mat2( 0.4,  0.4, 0.2,  0.6 );
const mat2 mtx = mat2( 0.2,  0.2, -0.4,  0.6 );
...
{
    q += 0.95*cos(vec2(0.17,0.19)*u_time + length( q )*1.40);

    q *= 0.7 + 0.01*cos(0.05);

    o = 0.5 + 0.5*fbm4_2( q );

    o +=  cos(0.1*u_time)+sin(0.05*u_time)+0.3*sin(vec2(0.61,0.73)*u_time*length( o ));
...
void main( )
{
    ...
    //col = mix( vec3(0.2,0.2,0.2), vec3(0.3,0.05,0.05), f );
    col = mix( vec3(0.2,0.2,0.2), vec3(0.3,0.3,0.3), f );
    //col = mix( col, vec3(0.9,0.9,0.9), dot(n,n) );
    //col = mix( col, vec3(0.5,0.5,0.5), 0.5*o.y*o.y );
